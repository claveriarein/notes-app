<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex items-center gap-1.5 text-sm text-gray-400 hover:text-red-500 transition-colors">
    ...
    <div class="max-w-5xl mx-auto px-6 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-400 mt-1">{{ Auth::user()->name }}'s personal workspace</p>
        </div>
        ...
        <div class="flex items-center gap-3 mb-8">
            <div class="relative flex-1">
             ...
             <a href="{{ route('notes.create') }}" class="bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-sm font-semibold px-5 py-3 rounded-2xl transition-all duration-150 shadow-lg shadow-indigo-200 whitespace-nowrap flex items-center gap-2">
        ...
        @if ($notes->isEmpty())
            <div class="text-center py-24">
                ...
                <a href="{{ route('notes.create') }}" class="text-indigo-600 text-sm hover:underline mt-2 inline-block font-semibold">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-5 flex flex-col justify-between hover:shadow-md hover:border-indigo-100 transition-all duration-200 group">
                        ...
                        <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('notes.edit', $note->id) }}" class="inline-flex items-center gap-1 text-xs text-indigo-400 hover:text-indigo-600 font-medium transition-colors">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    Edit
                                </a>
                                <button wire:click="confirmDelete({{ $note->id }})" class="inline-flex items-center gap-1 text-xs text-red-300 hover:text-red-500 font-medium transition-colors">
        ...
        <div class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-6">
            <div class="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl">
                ...
                <div class="flex gap-3">
                    <button wire:click="cancelDelete" class="flex-1 px-4 py-3 rounded-2xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
                        Cancel
                    </button>
                    <button wire:click="delete" class="flex-1 px-4 py-3 rounded-2xl bg-red-500 hover:bg-red-600 text-white text-sm font-semibold transition-colors">
                        Delete
                    </button>
                </div>
            </div>
